...
impl WslProcess {
    ...
    async fn run_wsl(&self, args: &[&str]) -> Result<Output, WSLError> {
        self.run_wsl_with_timeout(args, config::load_or_create().timeouts.default_secs)
            .await
    }
    async fn run_wsl_with_timeout(
        &self,
        args: &[&str],
        timeout_secs: u64,
    ) -> Result<Output, WSLError> {
        let output = timeout(Duration::from_secs(timeout_secs), async {
            Command::new("wsl.exe").args(args).output().await
        })
        .await
        .map_err(|_| {
            WSLError::CommandFailed(std::io::Error::new(
        ...
        if !output.status.success() {
            ...
            return Err(WSLError::ProcessFailed { code, stderr });
        }
    ...
    async fn run_wsl_quick(&self, args: &[&str]) -> Result<Output, WSLError> {
        self.run_wsl_with_timeout(args, config::load_or_create().timeouts.quick_secs)
            .await
    }

    async fn run_wsl_long(&self, args: &[&str]) -> Result<Output, WSLError> {
        self.run_wsl_with_timeout(args, config::load_or_create().timeouts.long_secs)
            .await
    }
    ...
    pub async fn run_distro(&self, name: &str) -> Result<(), WSLError> {
        let distros = self.get_distros().await?;
        distro_exists(name, &distros)?;
        self.run_wsl(&["--distribution", name, "--", "/bin/true"])
            .await?;
        Ok(())
    }
    ...
    pub async fn run_custom_action(
        ...
        if !status.success() {
            return Err(WSLError::CommandFailed(std::io::Error::other(format!(
                "Custom action exited with {status}"
            ))));
        }
        Ok(())
    }
